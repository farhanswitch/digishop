...
func (m marketController) getUserNotificationsCtrl(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	var userData map[string]interface{}
	headerUserData := r.Header.Get("X-User-Data")
	if headerUserData == "" {
	...
	data, customErr := m.service.GetUserNotificationsSrv(userData["id"].(string))
	if customErr.Code > 0 {
		log.Println(customErr)
		w.WriteHeader(int(customErr.Code))
		fmt.Fprintf(w, `{"errors":"%s"}`, customErr.MessageToSend)
